...
class ImagingPublicHandler(ContextHandler):

    def compute_etag(self):
        if self.context.config.ENABLE_ETAGS:
            return super(ImagingPublicHandler, self).compute_etag()
        else:
            return None
    ...
    def check_image(self, kw):
        if self.context.config.MAX_ID_LENGTH > 0:
            # Check if an image with an uuid exists in storage
            exists = yield gen.maybe_future(self.context.modules.storage_public.exists(kw['public'][:self.context.config.MAX_ID_LENGTH]))
            if exists:
                kw['public'] = kw['public'][:self.context.config.MAX_ID_LENGTH]

        url = self.request.path

        kw['public'] = quote(kw['public'].encode('utf-8'))
        if not self.validate(kw['public']):
            self._error(400, 'No original image was specified in the given URL')
            return
        ...
        if url_signature:
            ...
            if not valid and self.context.config.STORES_CRYPTO_KEY_FOR_EACH_IMAGE:
                # Retrieves security key for this image if it has been seen before
                security_key = yield gen.maybe_future(self.context.modules.storage_public.get_crypto(self.context.request.image_url))
                if security_key is not None:
                    signer = self.context.modules.url_signer(security_key)
                    valid = signer.validate(url_signature, url_to_validate)
