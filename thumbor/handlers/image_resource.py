...
from thumbor.handlers import ImageApiHandler, ImagePublicApiHandler
...
class ImagePublicResourceHandler(ImagePublicApiHandler):

    @gen.coroutine
    def check_resource(self, id):
        id = id[:self.context.config.MAX_ID_LENGTH]
        # Check if image exists
        exists = yield gen.maybe_future(self.context.modules.storage_public.exists(id))

        if exists:
            body = yield gen.maybe_future(self.context.modules.storage_public.get(id))
            self.set_status(200)
    ...
    def delete(self, id):
        ...
        exists = yield gen.maybe_future(self.context.modules.storage_public.exists(id))
        if exists:
            self.context.modules.storage_public.remove(id)
            self.set_status(204)
        else:
            self._error(404, 'Image not found at the given URL')
